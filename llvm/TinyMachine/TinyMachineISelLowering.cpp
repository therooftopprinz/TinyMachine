...
TinyMachineTargetLowering::TinyMachineTargetLowering(
  ...
  // RV64-style integer promotion: narrow scalar ints become i64 in registers.
  for (MVT VT : {MVT::i1, MVT::i8, MVT::i16, MVT::i32}) {
    setTypeAction(VT, TypePromoteInteger);
    AddPromotedToType(VT, MVT::i64);
  }
...
SDValue TinyMachineTargetLowering::LowerReturn(
  ...
  if (!Outs.empty()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, TinyMachine::A, OutVals[0], Glue);
